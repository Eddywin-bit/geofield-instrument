    ...
    const applyAll = () => {
      ...
      if (vectorBase) {
        ...
        if (beforeId) {
          ...
          // Lift the Natural Earth world borders/labels to the same insertion
          // point (above all basemap fills, below roads/labels). They default
          // to the very bottom, under the basemap's land fill, so they only
          // showed where the tile is blank; with the basemap's own borders and
          // country labels gated to zoom >= WORLD_CONTEXT_MAXZOOM, this makes
          // Natural Earth the single visible world source below that zoom -
          // everywhere, not just the blank regions - so there is no seam.
          for (const id of ["world-countries-line", "world-countries-label"]) {
            if (map.getLayer(id)) map.moveLayer(id, beforeId);
          }
        }
